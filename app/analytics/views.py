...
from .services.yandex_api import get_account_counters, get_visits, get_hits_counters, get_views_counters
...
def start_yandex_analytics(request, pk):
    ...
    # get and set hits counters
    domain_list = get_ya_user_counters(request)
    if get_views_counters(domain_list):
        messages.success(request, 'Yandex hits successfully set')
    else:
        messages.error(request, 'Yandex views failed set')
